...
THRESHOLDS = {
    ...
    # CVD (Taker Buy/Sell Ratio) - BTC icin
    "cvd_buy_strong":  55.0,    # Taker buy %55'in uzeri = saglam alim baskisi
    "cvd_sell_strong": 45.0,    # Taker buy %45'in alti = saglam satis baskisi
    # Pozisyon tavsiye esikleri (mevcut pozisyona karsi sinyal gucu)
    "counter_weak":     1.0,    # Bu kadar uzeri: stop yaklastir
    "counter_medium":   1.5,    # Bu kadar uzeri: kismi kapat
    "counter_strong":   2.5,    # Bu kadar uzeri: tamamen kapat / kar al
    "same_strong":      2.5,    # Pozisyon yonunde guclu sinyal: artirma firsati
}
...
def score_cvd(d):
    ...
    Yuksek taker buy = saglam alim baskisi (boga)
    Dusuk taker buy = saglam satis baskisi (ayi)
    OI/Fiyat ile birlestiginde guc onayi verir."""
...
def compute_tf_verdict(d):
    ...
    s_c, r_c = score_cvd(d)
    total = s_oi + s_w + s_r + s_f + s_c
    # 5 bilesen oldugu icin esikler de yukseldi
    verdict = "LONG" if total >= 2 else ("SHORT" if total <= -2 else "FLAT")
    reasons = [r for r in [r_oi, r_w, r_r, r_f, r_c] if r]
    return {"scores": {"oiPrice": s_oi, "whale": s_w, "retail": s_r,
                        "funding": s_f, "cvd": s_c, "total": total},
