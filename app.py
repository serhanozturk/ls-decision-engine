...
def run_backtest(symbol, days=7):
    ...
    for h1 in series_1h:
        ...
        decision = decide(symbol, tf_results, "flat", None, h1["priceNow"])

        # Sadece anlamli kararlari kaydet
        saveable = {"LONG_OPEN", "SHORT_OPEN", "CLOSE", "CLOSE_TP", "CLOSE_SL",
                    "PARTIAL_CLOSE", "TIGHTEN_STOP", "ADD_POSITION"}
        if decision.get("action") not in saveable:
            continue
    ...
    # series_1h sirali oldugu icin, sinyal_ts'in + N saat sonrasini bul
    ...
    for sig in signals:
        ...
        for label, hours in [("pnl1h", 1), ("pnl4h", 4), ("pnl24h", 24)]:
            ...
            if future and sig_price:
                ...
                # LONG sinyal -> fiyat artarsa kar, SHORT -> tersi
                if sig["action"] in ("LONG_OPEN", "ADD_POSITION") and "long" in sig["actionLabel"].lower():
...
function renderBacktest(data) {
  ...
  if (data.candles && data.candles.length) {
    ...
    const markers = (data.signals || []).map(sig => {
      ...
      if (sig.action === 'LONG_OPEN' || sig.action === 'ADD_POSITION') {
        position = 'belowBar'; color = '#00d09c'; shape = 'arrowUp';
        text = sig.action === 'LONG_OPEN' ? 'L' : 'L+';
      } else if (sig.action === 'SHORT_OPEN') {
        position = 'aboveBar'; color = '#ff4d6d'; shape = 'arrowDown'; text = 'S';
      } else if (sig.action.includes('CLOSE')) {
        position = 'aboveBar'; color = '#ffb83d'; shape = 'circle'; text = 'X';
      } else {
        return null;
      }
  ...
  function pnlCell(p) {
    ...
    const sign = p >= 0 ? '+' : '';
    return `<span class="pnl ${cls}">${sign}${p}%</span>`;
  }
  ...
  listEl.innerHTML = '<div class="bt-sig-row header">' +
    ...
    data.signals.reverse().map(sig => {
      ...
      return `<div class="bt-sig-row">
        ...
        <span class="pnl pnl-1h">${pnlCell(sig.pnl1h).replace('class="pnl ', 'class="').replace('pnl-1h', '')}</span>
        <span>${pnlCell(sig.pnl4h)}</span>
