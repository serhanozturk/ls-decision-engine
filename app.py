...
# Swing trader profili: 1h entry TF, 4h trend takibi, 1d makro
TF_WEIGHTS = {"15m": 0.5, "1h": 1.5, "4h": 2.0, "1d": 1.5}
# Toplam: 5.5 (eski toplam 4.5 idi)

# Esikler swing trader + killshot kuralina gore kalibre edildi.
# Killshot tetiklendiginde skor +2/-2 verir, dolayisiyla esikler daha yuksek.
THRESHOLDS = {
    # Karar esikleri (yeni toplam agirlik 5.5 + killshot ±2)
    "long_open":   5.0,
    "long_hold":   2.0,
    "short_hold": -2.0,
    "short_open": -5.0,
    # Whale-retail delta (senin keskin kuralin: ±3)
    "whale_long":   3.0,
    "whale_short": -3.0,
    # Killshot: ekstrem divergence + retail trendi
    "killshot_delta_long":   3.0,    # whale delta >= +3
    "killshot_delta_short": -3.0,    # whale delta <= -3
    "killshot_retail_trend": 0.3,    # retail %0.3+ degisim = "trend var" kabul
    # Retail asiri pozisyon (ek counter sinyal)
    ...
    # Pozisyon tavsiye esikleri (mevcut pozisyona karsi sinyal gucu)
    "counter_weak":     2.0,    # Stop yaklastir
    "counter_medium":   3.0,    # Kismi kapat %50
    "counter_strong":   5.0,    # Tamamen kapat / kar al
    "same_strong":      5.0,    # Pozisyon yonunde guclu sinyal: artirma firsati
}
...
def score_killshot(d):
    ...
    # Eger trend verisi yoksa, killshot tetiklenmez (guvenli taraf)
    if retail_prev is None:
        return 0, None

    retail_trend = retail - retail_prev  # pozitifse retail long yukseliyor
    threshold = THRESHOLDS["killshot_retail_trend"]

    # KESIN SHORT: whale shortta, retail long yukseliyor
    ...
    # KESIN LONG: whale longda, retail long azaliyor (yani short artiyor)
...
def compute_tf_verdict(d):
    ...
    s_c, r_c = score_cvd(d)
    s_k, r_k = score_killshot(d)  # ±2 verebilir
...
<div class="wrap">
  ...
  <div class="info">
    ...
    <b>NASIL CALISIR?</b> 4 TF (15m, 1h, 4h, 1d) skorlanir, agirlikli toplanir (1d/4h x1.5, 1h x1.0, 15m x0.5).<br><br>
