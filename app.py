...
# Backtest sonuc cache (5 gunde 1 calistirilacak, uzun cache ban korumasi)
_bt_cache = {}  # key -> (timestamp, result)
_BT_CACHE_TTL = 10800  # 3 saat

def _bt_fetch_single(base_url, sym, period, limit):
    """futures/data tek istek (max 500). Pagination YOK - ban korumasi icin hafif.
    eski->yeni sirali liste doner."""
    limit = min(500, limit)
...
def run_backtest(symbol, days=22):
    ...
    if _binance_banned():
        raise RuntimeError("Binance gecici banli, backtest simdi yapilamaz. Birkac saat sonra dene.")

    # futures/data tek istek max 500 nokta = ~20 gun. Pagination yok (ban korumasi).
    days = min(days, 20)
    bars = min(days * 24, 500)           # max 500 (tek sayfa, ~20 gun)
    kline_need = bars + 370              # EMA365 isinmasi icin ek
    kline_need = min(kline_need, 1500)   # Binance kline limiti

    # 1) Kline (fiyat + EMA365/7/30 icin) - tek istek
    ...
    # 2) Whale/retail/OI/CVD - TEK istek (500 nokta), aralarinda gecikme
    g_data = _bt_fetch_single("https://fapi.binance.com/futures/data/globalLongShortAccountRatio", sym, "1h", bars)
    _time.sleep(1.5)
    t_data = _bt_fetch_single("https://fapi.binance.com/futures/data/topLongShortPositionRatio", sym, "1h", bars)
    _time.sleep(1.5)
    oi_data = _bt_fetch_single("https://fapi.binance.com/futures/data/openInterestHist", sym, "1h", bars)
    _time.sleep(1.5)
    cvd_data = _bt_fetch_single("https://fapi.binance.com/futures/data/takerlongshortRatio", sym, "1h", bars)
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/api/backtest":
            symbol = (q.get("symbol", ["BTC"])[0] or "BTC").strip().upper()
            try:
                days = max(5, min(int(q.get("days", ["20"])[0]), 20))
            except ValueError:
                days = 22
            try:
...
<div class="wrap">
  ...
  <div class="controls">
    ...
    <div class="input-group">
      ...
      <div class="day-btns">
        ...
        <button class="day-btn active" data-days="20">20</button>
      </div>
    </div>
  ...
  <div class="info">
    ...
    <b>SINIR:</b> Binance whale/retail verisi tek istekte max 500 nokta (~20 gun). EMA365 icin ek 370 bar isinma cekilir. Sonuc 3 saat cache'lenir. Ban korumasi icin 5 gunde 1 calistirman onerilir.<br><br>
