...
def fetch_h1_data(sym):
    ...
    # EMA7/30 cross durumu + taze kesisim (1 mum teyitli)
    ema_cross = None       # mevcut yon: BULL (7>30) / BEAR (7<30)
    ema_fresh_cross = None # taze kesisim + teyit: BULL_CROSS / BEAR_CROSS
    if ema7[-1] and ema30[-1]:
        ema_cross = "BULL" if ema7[-1] > ema30[-1] else "BEAR"
        # Taze kesisim: onceki mumda ters, son 2 mumda ayni taraf (teyit)
        if ema7[-2] and ema30[-2] and ema7[-3] and ema30[-3]:
            now_above = ema7[-1] > ema30[-1]
            prev_above = ema7[-2] > ema30[-2]
            prev2_above = ema7[-3] > ema30[-3]
            # Yukari kesisim: 2 mum once altta, simdi ustte (kesisim oldu + teyit)
            if now_above and prev_above and not prev2_above:
                ema_fresh_cross = "BULL_CROSS"
            elif not now_above and not prev_above and prev2_above:
                ema_fresh_cross = "BEAR_CROSS"
...
def build_reasons(d, c, wr_delta, retail_trend):
    ...
    if d.get("aboveEma365"):
        r.append(f"Fiyat EMA365 ustunde (+{d.get('ema365Dist')}%) - BOGA rejim")
    elif d.get("belowEma365"):
        r.append(f"Fiyat EMA365 altinda ({d.get('ema365Dist')}%) - AYI rejim")
    else:
        r.append(f"Fiyat EMA365'e yakin ({d.get('ema365Dist')}%) - rejim belirsiz")

    # W/R
    if wr_delta is not None:
        if wr_delta >= THRESHOLDS["wr_bull"]:
            r.append(f"W/R +{wr_delta:.1f} (whale long baskin)")
        elif wr_delta <= THRESHOLDS["wr_bear"]:
            r.append(f"W/R {wr_delta:.1f} (whale short baskin)")
        else:
            r.append(f"W/R {wr_delta:+.1f} (notr)")

    # Retail trend
    if retail_trend is not None:
        if retail_trend >= THRESHOLDS["retail_trend_min"]:
            r.append(f"Retail long artiyor (+{retail_trend:.2f})")
        elif retail_trend <= -THRESHOLDS["retail_trend_min"]:
            r.append(f"Retail long dusuyor ({retail_trend:.2f})")

    # OI + fiyat
    if oi is not None and pr is not None:
        if oi >= THRESHOLDS["oi_min"] and pr >= THRESHOLDS["price_min"]:
            r.append(f"OI +{oi:.2f}% + fiyat +{pr:.2f}% = saglam yukselis")
        elif oi >= THRESHOLDS["oi_min"] and pr <= -THRESHOLDS["price_min"]:
            r.append(f"OI +{oi:.2f}% + fiyat {pr:.2f}% = saglam dusus")

    # CVD
    if cvd is not None:
        if cvd >= THRESHOLDS["cvd_buy"]:
            r.append(f"CVD %{cvd:.1f} (alim baskisi)")
        elif cvd <= THRESHOLDS["cvd_sell"]:
            r.append(f"CVD %{cvd:.1f} (satim baskisi)")

    # EMA cross
    if d.get("emaFreshCross") == "BULL_CROSS":
        r.append("EMA7 EMA30'u YUKARI kesti (taze)")
    elif d.get("emaFreshCross") == "BEAR_CROSS":
        r.append("EMA7 EMA30'u ASAGI kesti (taze)")
...
def decide(symbol, d, user_position, entry_price=None):
    ...
    reasons = build_reasons(d, c, wr_delta, retail_trend)
    ...
    fresh = d.get("emaFreshCross")

    # ============ POZISYON BAZLI KARAR ============
    if user_position == "long":
        action, label, subtitle = _decide_long(bull_score, bear_score, fresh, d, pnl_pct, regime)
    elif user_position == "short":
        action, label, subtitle = _decide_short(bull_score, bear_score, fresh, d, pnl_pct, regime)
    else:
        action, label, subtitle = _decide_flat(bull_score, bear_score, is_flat, regime)
...
def _decide_flat(bull, bear, is_flat, regime):
    """FLAT iken giris karari."""
    if is_flat:
        return "FLAT", "YATAY - SINYAL YOK", f"Piyasa kararsiz (boga {bull}/5, ayi {bear}/5)"
    # Boga baskin
    if bull >= SCORE_STRONG:
        return "LONG_OPEN", "KESIN BOGA - LONG AC", f"5/5 boga kosulu saglandi"
    if bull >= SCORE_OPEN:
        return "LONG_OPEN", "LONG AC", f"{bull}/5 boga kosulu - guclu sinyal"
    if bull >= SCORE_GAMBLE and bull > bear:
        return "GAMBLE_LONG", "GAMBLE MODU - LONG", f"{bull}/5 boga - manuel kontrol et"
    # Ayi baskin
    if bear >= SCORE_STRONG:
        return "SHORT_OPEN", "KESIN AYI - SHORT AC", f"5/5 ayi kosulu saglandi"
    if bear >= SCORE_OPEN:
        return "SHORT_OPEN", "SHORT AC", f"{bear}/5 ayi kosulu - guclu sinyal"
    if bear >= SCORE_GAMBLE and bear > bull:
        return "GAMBLE_SHORT", "GAMBLE MODU - SHORT", f"{bear}/5 ayi - manuel kontrol et"
    return "FLAT", "BEKLE", f"Yeterli sinyal yok (boga {bull}/5, ayi {bear}/5)"


def _decide_long(bull, bear, fresh, d, pnl_pct, regime):
    """LONG pozisyonda. EMA7/30 asagi kesti -> KAPAT. Ayi guclenirse -> KAPAT."""
    pnl_txt = f" (PnL {pnl_pct:+.2f}%)" if pnl_pct is not None else ""
    # Cikis 1: EMA7/30 asagi kesti (taze, teyitli) - momentum dondu
    if fresh == "BEAR_CROSS":
        if pnl_pct is not None and pnl_pct > 0:
            return "CLOSE_TP", "KAR AL - LONG KAPAT", f"EMA7 EMA30'u asagi kesti, momentum dondu{pnl_txt}"
        else:
            return "CLOSE_SL", "ZARARI KES - LONG KAPAT", f"EMA7 EMA30'u asagi kesti{pnl_txt}"
    # Cikis 2: Ayi skoru guclu (rejim donuyor)
    if bear >= SCORE_OPEN:
        if pnl_pct is not None and pnl_pct > 0:
            return "CLOSE_TP", "KAR AL - LONG KAPAT", f"Ayi sinyali guclendi ({bear}/5){pnl_txt}"
        else:
            return "CLOSE_SL", "ZARARI KES - LONG KAPAT", f"Ayi sinyali guclendi ({bear}/5){pnl_txt}"
    # Cikis 3: Rejim AYI + boga destegi yok (<=2) - trade mantigi cokmus
    if regime == "AYI" and bull <= 2:
        if pnl_pct is not None and pnl_pct > 0:
            return "CLOSE_TP", "KAR AL - LONG KAPAT", f"Rejim AYI, long destegi yok (boga {bull}/5){pnl_txt}"
        else:
            return "CLOSE_SL", "ZARARI KES - LONG KAPAT", f"Rejim AYI, long destegi yok (boga {bull}/5){pnl_txt}"
    # Hafif ters
    if bear >= SCORE_GAMBLE:
        return "TIGHTEN_STOP", "STOPU YAKLASTIR", f"Ayi sinyali beliriyor ({bear}/5), stop'u koru{pnl_txt}"
    # Devam
    return "HOLD", "TUT", f"Long korunuyor (boga {bull}/5){pnl_txt}"


def _decide_short(bull, bear, fresh, d, pnl_pct, regime):
    """SHORT pozisyonda. EMA7/30 yukari kesti -> KAPAT. Boga guclenirse -> KAPAT."""
    pnl_txt = f" (PnL {pnl_pct:+.2f}%)" if pnl_pct is not None else ""
    # Cikis 1: EMA7/30 yukari kesti (taze, teyitli)
    if fresh == "BULL_CROSS":
        if pnl_pct is not None and pnl_pct > 0:
            return "CLOSE_TP", "KAR AL - SHORT KAPAT", f"EMA7 EMA30'u yukari kesti, momentum dondu{pnl_txt}"
        else:
            return "CLOSE_SL", "ZARARI KES - SHORT KAPAT", f"EMA7 EMA30'u yukari kesti{pnl_txt}"
    # Cikis 2: Boga skoru guclu
    if bull >= SCORE_OPEN:
        if pnl_pct is not None and pnl_pct > 0:
            return "CLOSE_TP", "KAR AL - SHORT KAPAT", f"Boga sinyali guclendi ({bull}/5){pnl_txt}"
        else:
            return "CLOSE_SL", "ZARARI KES - SHORT KAPAT", f"Boga sinyali guclendi ({bull}/5){pnl_txt}"
    # Cikis 3: Rejim BOGA + ayi destegi yok (<=2) - trade mantigi cokmus
    if regime == "BOGA" and bear <= 2:
        if pnl_pct is not None and pnl_pct > 0:
            return "CLOSE_TP", "KAR AL - SHORT KAPAT", f"Rejim BOGA, short destegi yok (ayi {bear}/5){pnl_txt}"
        else:
            return "CLOSE_SL", "ZARARI KES - SHORT KAPAT", f"Rejim BOGA, short destegi yok (ayi {bear}/5){pnl_txt}"
    if bull >= SCORE_GAMBLE:
        return "TIGHTEN_STOP", "STOPU YAKLASTIR", f"Boga sinyali beliriyor ({bull}/5), stop'u koru{pnl_txt}"
    return "HOLD", "TUT", f"Short korunuyor (ayi {bear}/5){pnl_txt}"
...
def run_backtest(symbol, days=22):
    ...
    for i in range(test_start, len(closes)):
        ...
        # EMA7/30 cross + taze (1 mum teyit)
        ema_cross = ema_fresh = None
        if ema7_arr[i] and ema30_arr[i]:
            ema_cross = "BULL" if ema7_arr[i] > ema30_arr[i] else "BEAR"
            if i >= 2 and ema7_arr[i-1] and ema30_arr[i-1] and ema7_arr[i-2] and ema30_arr[i-2]:
                na = ema7_arr[i] > ema30_arr[i]
                pa = ema7_arr[i-1] > ema30_arr[i-1]
                p2a = ema7_arr[i-2] > ema30_arr[i-2]
                if na and pa and not p2a: ema_fresh = "BULL_CROSS"
                elif not na and not pa and p2a: ema_fresh = "BEAR_CROSS"
        ...
        d = {
            ...
            "emaCross": ema_cross, "emaFreshCross": ema_fresh,
        }

        dec = decide(symbol, d, "flat")  # FLAT giris simulasyonu

        regime_series.append({"t": int(ts/1000), "price": price_now,
                              "regime": dec.get("regime"), "ema365": round(e365,2) if e365 else None})

        # Sadece acilis sinyalleri (giris simulasyonu)
        if dec.get("action") in ("LONG_OPEN", "SHORT_OPEN", "GAMBLE_LONG", "GAMBLE_SHORT"):
            signals.append({
                "i": i, "ts": int(ts/1000), "action": dec["action"],
                "actionLabel": dec["actionLabel"], "regime": dec["regime"],
                "bull": dec["bullScore"], "bear": dec["bearScore"],
                "price": price_now,
            })

    # 4) Her sinyal icin PnL (1h/4h/24h sonra)
    for s in signals:
        i = s["i"]
        is_long = "LONG" in s["action"]
        for label, fwd in [("pnl1h", 1), ("pnl4h", 4), ("pnl24h", 24)]:
            j = i + fwd
            if j < len(closes):
                future = closes[j]
                if is_long:
                    s[label] = round((future - s["price"]) / s["price"] * 100, 2)
                else:
                    s[label] = round((s["price"] - future) / s["price"] * 100, 2)
            else:
                s[label] = None
        del s["i"]  # frontend'e gerek yok

    # 5) Istatistikler - genel + rejim bazli
    def stats(sigs, key):
        valid = [x for x in sigs if x.get(key) is not None]
        if not valid: return None
        wins = sum(1 for x in valid if x[key] > 0)
        return {"n": len(valid), "wins": wins,
                "winRate": round(wins/len(valid)*100, 1),
                "avgPnl": round(sum(x[key] for x in valid)/len(valid), 2)}

    longs = [s for s in signals if "LONG" in s["action"]]
    shorts = [s for s in signals if "SHORT" in s["action"]]
    gambles = [s for s in signals if "GAMBLE" in s["action"]]
    strong = [s for s in signals if "GAMBLE" not in s["action"]]

    summary = {
        "totalSignals": len(signals),
        "longCount": len(longs), "shortCount": len(shorts),
        "gambleCount": len(gambles), "strongCount": len(strong),
        "long_1h": stats(longs, "pnl1h"), "long_4h": stats(longs, "pnl4h"), "long_24h": stats(longs, "pnl24h"),
        "short_1h": stats(shorts, "pnl1h"), "short_4h": stats(shorts, "pnl4h"), "short_24h": stats(shorts, "pnl24h"),
        "strong_24h": stats(strong, "pnl24h"), "gamble_24h": stats(gambles, "pnl24h"),
    }
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }
  ...
  .reasoning li { font-size:11px; color:var(--text); padding:3px 0; padding-left:14px; position:relative; }
...
function renderDecision(symbol, data) {
  ...
  if (dec.reasons && dec.reasons.length) {
    rsn.style.display='';
    document.getElementById('reasoningList').innerHTML = dec.reasons.map(r=>`<li>${r}</li>`).join('');
...
<div class="wrap">
  ...
  <h2 class="section"><span class="arrow">&#9656;</span>WIN RATE TABLOSU (rejim bazli)</h2>
  <table class="wr-table" id="wrTable">
    <thead><tr><th>YON</th><th>1H</th><th>4H</th><th>24H</th><th>SAYI</th><th>ORT PnL (24h)</th></tr></thead>
  ...
  <div class="chart-wrap">
    ...
    <div class="chart-legend">
      <span><span class="dot" style="background:var(--green)"></span>LONG sinyal</span>
      <span><span class="dot" style="background:var(--red)"></span>SHORT sinyal</span>
      <span><span class="dot" style="background:var(--purple)"></span>GAMBLE</span>
      <span><span class="dot" style="background:var(--amber)"></span>EMA365</span>
    </div>
  </div>

  <h2 class="section"><span class="arrow">&#9656;</span>SINYAL LISTESI</h2>
  ...
  <div class="info">
    ...
    <b>NASIL CALISIR:</b> Secilen gun kadar gecmis 1h bar cekilir. Her bar icin EMA365 rejim + 5 kosul hesaplanir, FLAT giris simule edilir. Acilan sinyallerin 1h/4h/24h sonraki PnL'i olculur.<br><br>
...
async function runBacktest() {
  ...
  try {
    ...
    renderResults(data);
    status.innerHTML = `<b>${sym}</b> &middot; ${data.barsCovered} bar tarandi &middot; ${data.summary.totalSignals} sinyal bulundu${data.cached ? ' &middot; (cache)' : ''}`;
  } catch (e) {
...
function renderResults(data) {
  ...
  const l24 = s.long_24h, sh24 = s.short_24h, st24 = s.strong_24h;
  sg.innerHTML = `
    <div class="stat"><div class="stat-label">Toplam Sinyal</div><div class="stat-value neutral">${s.totalSignals}</div><div class="stat-sub">${data.days} gun / ${data.barsCovered} bar</div></div>
    <div class="stat"><div class="stat-label">Long / Short</div><div class="stat-value neutral">${s.longCount}/${s.shortCount}</div><div class="stat-sub">${s.strongCount} guclu, ${s.gambleCount} gamble</div></div>
    <div class="stat"><div class="stat-label">Long Win (24h)</div><div class="stat-value ${l24&&l24.winRate>=50?'pos':'neg'}">${l24?l24.winRate+'%':'--'}</div><div class="stat-sub">${l24?l24.wins+'/'+l24.n+' &middot; '+(l24.avgPnl>=0?'+':'')+l24.avgPnl+'%':'veri yok'}</div></div>
    <div class="stat"><div class="stat-label">Short Win (24h)</div><div class="stat-value ${sh24&&sh24.winRate>=50?'pos':'neg'}">${sh24?sh24.winRate+'%':'--'}</div><div class="stat-sub">${sh24?sh24.wins+'/'+sh24.n+' &middot; '+(sh24.avgPnl>=0?'+':'')+sh24.avgPnl+'%':'veri yok'}</div></div>
    <div class="stat"><div class="stat-label">Guclu Sinyal (24h)</div><div class="stat-value ${st24&&st24.winRate>=50?'pos':'neg'}">${st24?st24.winRate+'%':'--'}</div><div class="stat-sub">4-5/5 sinyaller</div></div>
  `;

  // Win rate tablosu
  const wb = document.getElementById('wrBody');
  function row(label, cls, st1, st4, st24v, cnt) {
    return `<tr>
      <td class="${cls}">${label}</td>
      <td>${wrCell(st1,'winRate')}</td>
      <td>${wrCell(st4,'winRate')}</td>
      <td>${wrCell(st24v,'winRate')}</td>
      <td>${cnt}</td>
      <td>${st24v?(st24v.avgPnl>=0?'<span class="wr-pos">+':'<span class="wr-neg">')+st24v.avgPnl+'%</span>':'<span class="wr-dim">--</span>'}</td>
    </tr>`;
  }
  wb.innerHTML =
    row('LONG (tum)','long', s.long_1h, s.long_4h, s.long_24h, s.longCount) +
    row('SHORT (tum)','short', s.short_1h, s.short_4h, s.short_24h, s.shortCount) +
    row('GUCLU (4-5/5)','', null, null, s.strong_24h, s.strongCount) +
    row('GAMBLE (3/5)','', null, null, s.gamble_24h, s.gambleCount);
  ...
  const markers = data.signals.map(sig => {
    if (sig.action.includes('GAMBLE')) {
      const isLong = sig.action.includes('LONG');
      return { time: sig.ts, position: isLong?'belowBar':'aboveBar', color:'#b06dff',
               shape: isLong?'arrowUp':'arrowDown', text:'G' };
    }
    if (sig.action === 'LONG_OPEN') return { time: sig.ts, position:'belowBar', color:'#00d09c', shape:'arrowUp', text:'L' };
    if (sig.action === 'SHORT_OPEN') return { time: sig.ts, position:'aboveBar', color:'#ff4d6d', shape:'arrowDown', text:'S' };
    return null;
  ...
  if (!data.signals.length) { sl.innerHTML = '<div class="empty">Bu donemde acilis sinyali yok</div>'; return; }
  const head = `<div class="sig-head"><span>TARIH</span><span>SINYAL</span><span>SKOR</span><span class="h-regime">REJIM</span><span class="h-1h">+1H</span><span class="h-4h">+4H</span><span>+24H</span></div>`;
  const rows = data.signals.slice().reverse().map(sig => {
    let acls = 'a-long', sc = sig.bull;
    if (sig.action.includes('GAMBLE')) { acls='a-gamble'; sc=Math.max(sig.bull,sig.bear); }
    else if (sig.action.includes('SHORT')) { acls='a-short'; sc=sig.bear; }
    const rcls = sig.regime==='BOGA'?'r-boga':(sig.regime==='AYI'?'r-ayi':'');
    return `<div class="sig-row">
      <span>${fmtTime(sig.ts)}</span>
      <span class="${acls}">${sig.actionLabel}</span>
      <span>${sc}/5</span>
      <span class="c-regime ${rcls}">${sig.regime||'--'}</span>
      <span class="c-1h">${pnlCell(sig.pnl1h)}</span>
      <span class="c-4h">${pnlCell(sig.pnl4h)}</span>
      <span>${pnlCell(sig.pnl24h)}</span>
    </div>`;
  }).join('');
