...
HOST = "0.0.0.0"
USER_AGENT = "Mozilla/5.0 LSDecisionEngine/3.0"

TIMEFRAMES = ["15m", "1h", "4h", "1d"]
TF_WEIGHTS = {"15m": 0.5, "1h": 1.0, "4h": 1.5, "1d": 1.5}

# Esikler BTC verisine gore kalibre edilmistir.
# BTC funding rate genelde %0.005-0.02 araliginda; OI ve fiyat
# hareketleri saatlik bazda %0.5-2 araligindadir.
THRESHOLDS = {
    "long_open":   2.5,    # Eskiden 3.5 - cok zor tetikleniyordu
    "long_hold":   1.0,    # Eskiden 1.5
    "short_hold": -1.0,
    "short_open": -2.5,
    "whale_long":   5.0,   # Eskiden 10 - BTC'de whale-retail delta nadir 10'u gecer
    "whale_short": -5.0,
    "retail_extreme_long":  65.0,  # Eskiden 75 - BTC'de %75 nadir
    "retail_extreme_short": 35.0,
    "funding_extreme_pos":  0.015,  # Eskiden 0.05 - cok yuksek
    "funding_extreme_neg": -0.015,
    "oi_delta_strong":   0.5,   # Eskiden 2.0
    "price_delta_strong": 0.3,  # Eskiden 1.0
}
...
def binance_tf_data(sym, tf):
    ...
    url2 = f"https://fapi.binance.com/futures/data/topLongShortPositionRatio?symbol={sym}&period={p}&limit=1"
...
def compute_tf_verdict(d):
    ...
    total = s_oi + s_w + s_r + s_f
    verdict = "LONG" if total >= 2 else ("SHORT" if total <= -2 else "FLAT")
    reasons = [r for r in [r_oi, r_w, r_r, r_f] if r]
    return {"scores": {"oiPrice": s_oi, "whale": s_w, "retail": s_r, "funding": s_f, "total": total},
            "reasons": reasons, "verdict": verdict}


def decide(symbol, tf_results, user_position):
    weighted_total = 0.0
    weight_sum = 0.0
    all_reasons = []
    ...
    if weight_sum == 0:
        return {"action": "FLAT", "actionLabel": "VERI YOK",
                "subtitle": "Yeterli veri toplanamadi", "totalScore": 0.0, "reasons": []}
    total = weighted_total
    if total >= THRESHOLDS["long_open"]:
        action, label, subtitle = "LONG_OPEN", "LONG AC", "Guclu boga sinyali"
    elif total >= THRESHOLDS["long_hold"]:
        action, label, subtitle = "LONG_HOLD", "LONG TUT", "Boga egilim, long tutulabilir"
    elif total <= THRESHOLDS["short_open"]:
        action, label, subtitle = "SHORT_OPEN", "SHORT AC", "Guclu ayi sinyali"
    elif total <= THRESHOLDS["short_hold"]:
        action, label, subtitle = "SHORT_HOLD", "SHORT TUT", "Ayi egilim, short tutulabilir"
    else:
        action, label, subtitle = "FLAT", "FLAT", "Net sinyal yok, beklemede kal"
    if user_position == "long" and total <= THRESHOLDS["short_hold"]:
        action, label = "CLOSE", "LONG KAPAT"
        subtitle = "Mevcut long pozisyona karsi sinyal olustu"
    elif user_position == "short" and total >= THRESHOLDS["long_hold"]:
        action, label = "CLOSE", "SHORT KAPAT"
        subtitle = "Mevcut short pozisyona karsi sinyal olustu"
    return {"action": action, "actionLabel": label, "subtitle": subtitle,
            "totalScore": total, "reasons": all_reasons[:10]}


def analyze_symbol(symbol, user_position):
    ...
    decision = decide(symbol, tf_results, user_position)
    ...
    def do_GET(self):
        ...
        if path == "/api/analyze":
            ...
            if not symbol:
                ...
                return
            try:
                data = analyze_symbol(symbol, user_pos)
...
<style>
  ...
  * { box-sizing:border-box; margin:0; padding:0; -webkit-tap-highlight-color:transparent; }
  ...
  .decision.action-CLOSE::before { background:var(--amber); }
  .decision-head { display:flex; justify-content:space-between; align-items:baseline;
    margin-bottom:18px; gap:16px; flex-wrap:wrap; }
  .action-label { font-size:28px; font-weight:700; letter-spacing:0.05em; }
  .action-LONG_OPEN .action-label, .action-LONG_HOLD .action-label { color:var(--green); }
  .action-SHORT_OPEN .action-label, .action-SHORT_HOLD .action-label { color:var(--red); }
  .action-FLAT .action-label { color:var(--text-dim); }
  .action-CLOSE .action-label { color:var(--amber); }
  .score-pill { font-size:12px; color:var(--text-dim); letter-spacing:0.08em; }
  ...
  .matrix-row { display:grid; grid-template-columns:70px repeat(5, 1fr) 110px;
  ...
  @media (max-width:720px) {
    ...
    .matrix-row { grid-template-columns:50px repeat(5, 1fr) 80px; }
...
<div class="wrap">
  ...
  <div class="info">
    <b>BTC ICIN OPTIMIZE:</b> Esikler ve mantik BTC'nin tipik volatilitesi (OI &amp;%0.5, fiyat &amp;%0.3, funding &amp;%0.015) gozonune alinarak ayarlanmistir. ETH, SOL gibi buyuk coinlerde de calisir ama optimal degerler farkli olabilir.<br><br>
    <b>NASIL CALISIR?</b> 4 zaman dilimi (15m, 1h, 4h, 1d) analiz edilir. Her TF icin 4 metrik (OI/fiyat, whale-retail, retail asiri pozisyon, funding) skorlanir. Toplam skor agirlikli ortalamayla hesaplanir (1d ve 4h x1.5, 1h x1.0, 15m x0.5).<br><br>
    <b>POZISYON:</b> Manuel olarak isaretle. Sistem ters yonde guclu sinyal alirsa "KAPAT" onerir.<br><br>
...
function renderDecision(symbol, decision, tfData) {
  ...
  body.innerHTML = TIMEFRAMES.map(tf => {
    ...
    if (!d || !d.ok) {
      return `<div class="matrix-row">
        ...
        <div class="matrix-cell cell-zero" style="grid-column: span 6">veri yok</div>
      </div>`;
    }
    return `<div class="matrix-row">
      <div class="matrix-cell tf-label">${tf}</div>
...
async function run() {
  ...
  try {
    const r = await fetch('/api/analyze?symbol=' + encodeURIComponent(sym) +
                         '&position=' + encodeURIComponent(userPosition));
    if (!r.ok) throw new Error('HTTP ' + r.status);
    const data = await r.json();
    if (!data.ok) throw new Error(data.error || 'API hatasi');
    lastFetch = data;
    renderDecision(sym, data.decision, data.timeframes);
