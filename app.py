...
HOST = "0.0.0.0"
USER_AGENT = "Mozilla/5.0 LSDecisionEngine/9.0"
...
def fetch_h1_data(sym):
    """BTC H1 verisi: whale/retail (son + 2 mum once), OI, CVD, fiyat, EMA365/7/30."""
    p = "1h"

    # 1) Global L/S account ratio (son 3 nokta - 2 mum once icin)
    g = http_get(f"https://fapi.binance.com/futures/data/globalLongShortAccountRatio?symbol={sym}&period={p}&limit=3")
    if not g:
        raise RuntimeError("global ratio yok")
    global_long_now = float(g[-1]["longAccount"]) * 100
    global_long_2ago = float(g[-3]["longAccount"]) * 100 if len(g) >= 3 else float(g[0]["longAccount"]) * 100

    # 2) Top trader position ratio (whale)
    t = safe(lambda: http_get(f"https://fapi.binance.com/futures/data/topLongShortPositionRatio?symbol={sym}&period={p}&limit=3"), [])
    whale_now = float(t[-1]["longAccount"]) * 100 if t else None
    whale_2ago = float(t[-3]["longAccount"]) * 100 if t and len(t) >= 3 else (float(t[0]["longAccount"]) * 100 if t else None)
    ...
    # 3) OI (son 3 nokta)
    oi = safe(lambda: http_get(f"https://fapi.binance.com/futures/data/openInterestHist?symbol={sym}&period={p}&limit=3"), [])
    oi_now = float(oi[-1].get("sumOpenInterestValue") or 0) if oi else None
    oi_2ago = float(oi[-3].get("sumOpenInterestValue") or 0) if oi and len(oi) >= 3 else None
    oi_change = None
    if oi_now and oi_2ago and oi_2ago > 0:
        oi_change = (oi_now - oi_2ago) / oi_2ago * 100

    # 4) Fiyat - kline (EMA icin bol veri: 500 mum)
    kl = http_get(f"https://fapi.binance.com/fapi/v1/klines?symbol={sym}&interval={p}&limit=500")
    closes = [float(k[4]) for k in kl]
    price_now = closes[-1]
    price_2ago = closes[-3] if len(closes) >= 3 else closes[0]
    price_change = (price_now - price_2ago) / price_2ago * 100 if price_2ago > 0 else None

    # 5) CVD - taker buy/sell (3 nokta agirlikli ortalama)
    cvd = safe(lambda: http_get(f"https://fapi.binance.com/futures/data/takerlongshortRatio?symbol={sym}&period={p}&limit=3"), [])
    ...
    if cvd:
        ...
        pcts = [buy_pct(pt) for pt in cvd if buy_pct(pt) is not None]
        if pcts:
            if len(pcts) >= 3:
    ...
    # 7) EMA hesaplari
    ema365 = calc_ema(closes, 365)
    ema7 = calc_ema(closes, 7)
    ema30 = calc_ema(closes, 30)

    # Fiyat EMA365 ustunde/altinda kac mum? (son 2 mum kontrolu)
    ema365_now = ema365[-1]
    above_ema365_2mum = below_ema365_2mum = False
    if ema365_now and ema365[-2]:
        above_ema365_2mum = closes[-1] > ema365[-1] and closes[-2] > ema365[-2]
        below_ema365_2mum = closes[-1] < ema365[-1] and closes[-2] < ema365[-2]
